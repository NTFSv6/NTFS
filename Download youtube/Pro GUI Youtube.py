...
def select_path():
    ...
    path = filedialog.askdirectory()  # لتحديد مسار معين في اي مكان في الكمبيوتر
    path_label.config(text=path) # لكتابه المسار الذي تم تحديده
...
# image logo لوضع صوره بداخل الوجه المستخدمه
logo_img = PhotoImage(file='yt.png')
# resize
logo_img = logo_img.subsample(2, 2)  # للتحكم في حجم الصوره
canvas.create_image(250, 80, image=logo_img)  # لوضع الصوره

# link field
link_field = Entry(screen, width=40, font=('Arial', 15))  # عمل ادخال ال input
link_label = Label(screen, text="Enter Download Link: ",
                   font=('Arial', 15))  # لكتابه نص معين
